...
tables = [ "table_w_zstd_2", "table_w_lz4c", "table_w_none" ]
table=tables[0]
...
class BigtableDataProcessor:
    ...
    def decode_base64_value(self, value_b64):
    ...
    def extract_cell_data(self, cells):
        ...
        for cell in cells:
            ...
            decoded_value = self.decode_base64_value(value_b64)
            ...
            if family_name:
                ...
                if family_name in record['families']:
                    for cell in record['families'][family_name]:
                        rows.append({
                            ...
                            'value_b64': cell['value_b64']
                        })
                        #print(f"Executing CQL1: {row_key}, {family_name}, {cell['qualifier']}, {cell['timestamp']}, {cell['timestamp_micros']}, {cell['value_b64']}")
                        session.execute(cql_prepared, (row_key, family_name, cell['qualifier'], cell['timestamp'], cell['timestamp_micros'], cell['value_b64']))
            else:
                ...
                for family, cells in record['families'].items():
                    for cell in cells:
                        ...
                        #print(f"Executing CQL2: {row_key}, {family}, {cell['qualifier']}, {cell['timestamp']}, {cell['timestamp_micros']}, {cell['value_b64']}")
                        session.execute(cql_prepared, (row_key, family, cell['qualifier'], cell['timestamp'], cell['timestamp_micros'], cell['value_b64']))
...
def main():
    ...
    try:
        ...
        for family, count in list(analysis['family_distribution'].items()):
...
if __name__ == "__main__":
    ...
    cluster = Cluster(SCYLLA_IP, auth_provider=PlainTextAuthProvider(username='cassandra', password='cassandra'))
